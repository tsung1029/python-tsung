...
class Plot:
	...
	def __init__(self,text):
		...
		'dpi' : int , 'fig_size':float, 'fig_layout': int, 'fontsize': int, 'save_dir' : str, 'sim_dir' : str}
	...
	def get_data(self,file,file_num):
		...
		if(plot_type == 'raw'):
			...
			if(dim == 2):
				...
				hist, yedges,xedges = np.histogram2d(data1,data2,bins = bins, range = bounds, weights = np.abs(q_weight*norm))
