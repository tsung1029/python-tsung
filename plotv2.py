...
def main():
    ...
    if (dirs[len(dirs) - 1] != '/'):
        dirs = dirs + '/'
    subprocess.call(
        ["ffmpeg", "-framerate", "10", "-pattern_type", "glob", "-i", dirs + '*.png', '-c:v', 'libx264', '-vf',
         'scale=' + str(x) + ':' + str(y) + ' ,format=yuv420p', dirs + 'movie.mp4'])


def perform_ops(ops_name, data):
    if ops_name == 'abs':
        return np.abs(data)
    elif ops_name == 'square':
...
class Subplot(Plot):
    def __init__(self, text, num, general_params):
        ...
        self.types = {'folders': str, 'title': str, 'log_threshold': float, \
                      ...
                      'use_dir': str, 'linewidth': float, 'operation': str}
        self.left = 0
        self.right = 0
        self.general_dict = {}
        self.raw_edges = {}
