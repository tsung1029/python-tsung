...
def print_help():
    ...
    print '  --ocomp=[l,t1,t2]: which component to output'
...
if outdir[-1] != '/':
    outdir += '/'
# # SET DEFAULT OPTIONS
dir_ext, blockx, blocky, ifftdim, ocomp = '', None, None, None, None
for opt, arg in opts:
    if opt == '-h':
        print_help()
        sys.exit()
...
def foreach_decompose(file_num):
    ...
    if blockx or blocky:
        e1data = arraymask.mask(e1data, axes=[kx, ky], region=brg)
        e2data = arraymask.mask(e2data, axes=[kx, ky], region=brg)
    ...
    if ift:
        h5_output.data, e1data = (analysis.analysis(e1data - h5_output.data, ['ifft ' + ift, 'abs']),
    ...
    if ift:
        h5_output.data, e2data = (analysis.analysis(e2data - h5_output.data, ['ifft ' + ift, 'abs']),
                                  analysis.analysis(h5_output.data, ['ifft ' + ift, 'abs']))
    else:
    ...
    if 'l' in ocomp:
        h5_output.data = np.sqrt(np.square(e1data) + np.square(e2data))
